...
def province_predict(img_name,xx):
    with xx.as_default():
        with tf.Session(graph=xx) as sess:
            saver = tf.train.import_meta_graph("%smodel.ckpt.meta" % (SAVER_DIR))
            model_file = tf.train.latest_checkpoint(SAVER_DIR)
            saver.restore(sess, model_file)
            x = xx.get_tensor_by_name("Placeholder:0")
            keep_prob = xx.get_tensor_by_name("Placeholder_2:0")
            conv = xx.get_tensor_by_name('add_3:0')
